...
- `TextNode.toHTML()` escaped only the *first* occurrence of each markup character, because it used `String.replace` with string patterns rather than regular expressions, and escaped `&` after `<` and `>`, so any entity it produced was escaped again (`<` became `&amp;lt;`). This is the serializer behind the editor's clipboard `text/html` payload.
