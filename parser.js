...
function parseChapter(text) {
    ...
    while(lines.length > 0) {
        ...
        // Header
            ...
            elements.push(
                count === 1 ? 
                    <h2 key={"element" + key++}>{parseLine(trimmed, 0)}</h2> :
                count === 2 ? 
                    <h3 key={"element" + key++}>{parseLine(trimmed, 0)}</h3> :
                    <h4 key={"element" + key++}>{parseLine(trimmed, 0)}</h4>
            );
        ...
        // Bulleted list
            ...
            while(trimmed && trimmed.startsWith("* ")) {
                bullets.push(<li key={"bullet" + bullets.length}>{parseLine(trimmed.substring(1).trim(), 0)}</li>);
        ...
        // Numbered list
            ...
            while(trimmed && trimmed.match("^[0-9]+.*")) {
                items.push(<li key={"item" + items.length}>{this.parseLine(trimmed.substring(2).trim(), 0)}</li>);
                // Keep reading blank lines.
                do {
                    trimmed = lines.shift();
                    if(trimmed) trimmed = trimmed.trim();
        ...
        // If it didn't match any of the blocks, make a paragraph.
            ...
            sequence = sequence.concat(parseLine(line, sequence.length));
        }
