...
export function genericHashLink(As: typeof Link | typeof NavLink) {
  return React.forwardRef<HTMLAnchorElement, HashLinkProps>((props: HashLinkProps, ref) => {
    let linkHash = '';
    ...
    const passDownProps: { isActive?: Function } = {};
    if (As === NavLink) {
      passDownProps.isActive = (match: { isExact: boolean }, location: Location) =>
        match && match.isExact && location.hash === linkHash;
    }

    function handleClick(e: { button: number, defaultPrevented : boolean, metaKey: boolean, altKey: boolean, ctrlKey: boolean, shiftKey: boolean }) {
      reset();
