...
export type FormattedNodeParent = BlockNode | FormattedNode | SubSuperscriptNode | FootnoteNode | LinkNode | EmbedNode
...
export class FormattedNode extends Node {
    ...
    constructor(parent: FormattedNodeParent | undefined, format: Format, segments: FormattedNodeSegmentType[]) {
        ...
        segments.forEach(seg => seg.parent = this);
    }
        ...
        for(let i = 0; i < this.segments.length; i++) {
            ...
            if(node instanceof TextNode) {
                ...
                // Slurp the current node's text
                    ...
                    currentText.text = currentText.text + node.text;
    ...
    clean() {
        ...
        if(this.parent instanceof FormattedNode && this.format === this.parent.format) {
            this.segments.forEach(seg => seg.parent = this.parent);
            const index = this.parent.segments.indexOf(this);
            if(index < 0) throw Error("Uh oh, parentage is wrong.");
            this.parent.segments = this.parent.segments.slice(0, index).concat(this.segments).concat(this.parent.segments.slice(index + 1));
        }
    ...
    textIndexToCaret(index: number): Caret | undefined {
        ...
        for(let i = 0; i < text.length; i++) {
            const t = text[i];
            if(index >= currentIndex && index <= currentIndex + t.text.length)
                return { node: t, index: index - currentIndex };
            currentIndex += t.text.length;
        }
        return undefined;
    ...
    formatRange(range: CaretRange, format: Format): CaretRange {
        ...
        const selectedNodes = textNodes.filter(n => 
                ...
                (n === startNode && range.start.index < startNode.text.length) ||
        ...
        nodesToFormat.forEach(text => {
            ...
            if(apply) {
                const parent = text.parent as FormattedNode;
                const formattedMiddle = new FormattedNode(parent, format, []);
                const left = new TextNode(parent, text.text.substring(0, text === range.start.node ? range.start.index : 0), 0);
                const middle = new TextNode(formattedMiddle, text.text.substring(text === range.start.node ? range.start.index : 0, text === range.end.node ? range.end.index : text.text.length), 0);
                const right = new TextNode(parent, text.text.substring(text === range.end.node ? range.end.index : text.text.length), 0);
            ...
            //   3. •I think ••broadening participation in ••com•|•put•|•ing•• is cool.•
                ...
                const parent = text.parent as FormattedNode;
                const left = new TextNode(parent, text.text.substring(0, text === range.start.node ? range.start.index : 0), 0);
                const middle = new TextNode(parent, text.text.substring(text === range.start.node ? range.start.index : 0, text === range.end.node ? range.end.index : text.text.length), 0);
                const right = new TextNode(parent, text.text.substring(text === range.end.node ? range.end.index : text.text.length), 0);
