...
const STRING_REF = { legacy: 'Author, A. (2020). A cited work.' };
...
vi.mock('$lib/models/CRUD', async () => {
    ...
    const makeEditionWithStringRef = () =>
        makeEdition({ legacy: 'Author, A. (2020). A cited work.' });
    return {
        ...
        listenToEdition: (
            ...
            queueMicrotask(() => callback(makeEditionWithStringRef()));
            return () => undefined;
        },
        listenToChapters: (
            _bookID: string,
...
test('the write route renders an edition with a legacy string-form reference', async () => {
    const errors: string[] = [];
