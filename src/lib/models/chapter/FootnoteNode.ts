...
export default class FootnoteNode extends AtomNode<FormatNode> {
    ...
    toText(): string {
        return this.getMeta().toText();
    }
    toBookdown(): string {
        return `{${this.getMeta().toBookdown()}}`;
    }
    toHTML(): string {
    ...
    copy() {
        return new FootnoteNode(this.getMeta().copy()) as this;
    }
