...
import type { Format } from './FormatNode';
import type Caret from './Caret';
import { caretToIndex, indexToCaret, type CaretRange } from './Caret';
import BlockNode from './BlockNode';
import type Edit from './Edit';

export type ListNodeType = FormatNode | ListNode;

export default class ListNode extends BlockNode {
    ...
    isEmpty(): boolean {
        return (
            this.#items.length === 0 ||
            this.#items.every((i) => i instanceof ListNode && i.isEmpty())
        );
    }
    getItems() {
        return this.#items;
    ...
    getListItems() {
        return this.#items.filter((i) => i instanceof ListNode) as ListNode[];
    }
    getLength() {
        return this.#items.length;
    }
    getLastItem(): FormatNode | undefined {
        return this.getFirstLastItem(false);
    }
    getFirstItem(): FormatNode | undefined {
        return this.getFirstLastItem(true);
    }
    ...
    getFirstLastItem(first: boolean): FormatNode | undefined {
        ...
        if (last instanceof FormatNode) return last;
    ...
    getLevel(root: Node): number {
        ...
        if (parent instanceof ListNode) return parent.getLevel(root) + 1;
        else return 1;
    }
    ...
    withItemSplit(caret: Caret): ListNode | undefined {
        ...
        return this.withItemAfter(second, item)
            ?.withItemAfter(first, item)
            ?.withoutItem(item);
    }
    ...
    withNodeInserted(caret: Caret, node: Node): Edit {
        ...
        while (items.length > 0) {
            ...
            if (lastItem === anchor) {
                ...
                if (edit === undefined || !(edit.root instanceof FormatNode))
                    return;
                item = edit.root;
        ...
        const newCaret =
            lastItem instanceof FormatNode
                ? lastItem.getLastCaret()
                : lastItem.getLastItem()?.getLastCaret();
        if (newCaret === undefined) return;
    ...
    withRangeFormatted(range: CaretRange, format: Format | undefined): Edit {
        ...
        if (
            ...
            const itemIndex =
                itemEnd - (this.#items.length - newList.#items.length);
            if (itemIndex > 0 && newList.getLength() > 1) {
                ...
                return {
                    root: mergedEdit[0],
                    range: { start: mergedEdit[1], end: mergedEdit[1] },
                };
            }
        }
