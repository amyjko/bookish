...
export default class TextNode extends Node {
    ...
    copy() {
        return new TextNode(this.#text) as this;
    }
    ...
    nextWord(root: RootNode, index?: number): Caret {
        ...
        if (
            i < this.getLength() ||
            (i === this.getLength() && this.#text.charAt(i - 1).match(/\w/))
        )
        ...
        if (
            formatTextNodes &&
            nextNode &&
            this === formatTextNodes[formatTextNodes.length - 1]
        )
    ...
    previousWord(root: RootNode, index?: number): Caret {
        if (index === undefined) index = this.#text.length;
        ...
        if (formatTextNodes && previousNode && this === formatTextNodes[0])
            return {
                node: previousNode,
                index:
                    previousNode instanceof TextNode
                        ? previousNode.getLength()
                        : 0,
            };
        else if (previousWord === undefined) return { node: this, index: 0 };
