...
export default class QuoteNode extends BlocksNode {
    ...
    toText(): string {
        return (
            this.getBlocks()
                ...
                .join(' ') + (this.#credit ? ' ' + this.#credit.toText() : '')
        );
    }
    ...
    toBookdown(): string {
        return `"\n${this.getBlocks()
            ...
            this.#credit ? this.#credit.toBookdown() : ''
        }`;
    }
    toHTML() {
        return `"${super.toHTML()}"<center>${this.#credit.toHTML()}</center>`;
    }

    getChildren(): Node[] {
        return [...this.getBlocks(), this.#credit];
    }
    ...
    copy() {
        return new QuoteNode(this.getBlocks().map((b) => b.copy())) as this;
    }
    ...
    withChildReplaced(
        ...
        // Replace a block.
        if (replacement !== undefined && !(replacement instanceof BlockNode))
            return;
    ...
    withCredit(credit: FormatNode | undefined): QuoteNode {
        return new QuoteNode(this.getBlocks(), credit, this.#position);
    }
    withPosition(position: Position): QuoteNode {
        return new QuoteNode(this.getBlocks(), this.#credit, position);
    }
    ...
    withContentInRange(range: CaretRange): this | undefined {
        ...
        const newCredit =
            this.getBlocks().find((b) => b.contains(range.end.node)) !==
            undefined
                ? new FormatNode('', [new TextNode()])
