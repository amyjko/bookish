...
export default abstract class BlocksNode extends BlockNode {
    ...
    constructor(elements: BlockNode[]) {
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            if (
                ...
                previousBlock instanceof ListNode &&
                block.isNumbered() === previousBlock.isNumbered()
            ) {
                adjustedElements[adjustedElements.length - 1] =
                    previousBlock.withListAppended(block);
            }
        ...
        if (this.#blocks.length === 0) this.#blocks.push(new ParagraphNode());
    }
    ...
    getBlockAfter(anchor: BlockNode): BlockNode | undefined {
        const index = this.#blocks.indexOf(anchor);
        if (index < 0 || index > this.#blocks.length - 2) return undefined;
        return this.#blocks[index + 1];
    }
    ...
    getBlocksBetween(
        ...
        if (firstIndex !== undefined && lastIndex !== undefined) {
            ...
            for (let i = 0; i < this.#blocks.length; i++) {
                ...
                if (block === first) inside = true;
                if (inside) blocks.push(block);
                if (block === last) break;
            }
            return blocks;
        }
    ...
    getBlocksInRange(range: CaretRange): BlockNode[] | undefined {
        ...
        const endBlock =
            range.end === range.start
                ? startBlock
                : this.getBlockOfCaret(range.end);
        ...
        this.getNodes()
            ...
            .forEach((node) => {
                ...
                if (insideSelection && node instanceof BlockNode)
                    blocksToEdit.push(node);
                if (insideSelection && node === endBlock)
    ...
    getLastCaret(): Caret | undefined {
        ...
        return text.length === 0
            ? undefined
            : {
                  node: text[text.length - 1],
                  index: text[text.length - 1].getLength(),
              };
    }
    ...
    getAdjacentCaret(caret: Caret, next: boolean): Caret | undefined {
        ...
        const formats = this.getNodes().filter((n) => {
            ...
            return (
                (n instanceof TextNode && parent instanceof CodeNode) ||
                (n instanceof FormatNode &&
                    !(parent instanceof AtomNode) &&
                    !(parent instanceof FormatNode))
            );
        }) as (TextNode | FormatNode)[];

        const index = formats.indexOf(format);
        const adjacentFormat =
            index < 0 || formats.length === 0
                ? undefined
                : formats[index + (next ? 1 : -1)];
        if (adjacentFormat === undefined) return;
        return next
            ? adjacentFormat.getFirstCaret()
            : adjacentFormat.getLastCaret();
    }
    ...
    getSelectedFormatRange(
        ...
        if (formatIndex < startIndex || formatIndex > endIndex)
            return undefined;

        // Choose the portion of the format that is selected.
        const startCaret =
            formatIndex === startIndex ? range.start : format.getFirstCaret();
        const endCaret =
            formatIndex === endIndex ? range.end : format.getLastCaret();
    ...
    withSelectionSplit(range: CaretRange): Edit {
        ...
        if (
            ...
            if (edit === undefined || !(edit.root instanceof BlocksNode))
                return;
            blocks = edit.root;
    ...
    withoutAtom(atom: AtomNode<any>): Edit {
        ...
        const newCaret = {
            node: previous ? previous : next ? next : newText,
            index:
                previous && previous instanceof TextNode
                    ? previous.getLength()
                    : 0,
        };
    ...
    withRangeFormatted(range: CaretRange, format: Format | undefined): Edit {
        ...
        if (
            ...
            const selection = range.start.node
                .getFormatRoot(this)
                ?.getSelection();
        ...
        for (let i = 0; i < this.#blocks.length; i++) {
            ...
            if (containsStart || containsEnd || inRange) {
                ...
                const blockStart = containsStart
                    ? sortedRange.start
                    : block.getFirstCaret();
                const blockEnd = containsEnd
                    ? sortedRange.end
                    : block.getLastCaret();
                ...
                // Otherwise, format it.
                    ...
                    if (
                        ...
                        if (firstItem instanceof FormatNode) {
                            ...
                            const newListWithoutItem =
                                newBlock.withoutItem(firstItem);
                            if (newListWithoutItem === undefined) return;
                            newBlock = newListWithoutItem;

                            // Replace the revised list
                            const blocksWithList: BlocksNode | undefined =
                                newBlocks
                    ...
                    // Otherwise, just handle the new block in isolation.
                        ...
                        if (!newBlock.isEmpty()) formattedBlocks.push(newBlock);
            ...
            if (block.contains(sortedRange.end.node)) inRange = false;
        }
        ...
        if (format === undefined && formattedBlocks.length >= 2) {
            ...
            if (
                first instanceof ParagraphNode &&
                last instanceof ParagraphNode
            ) {
        ...
        const endCaret =
            format === undefined
                ? startCaret
                : indexToCaret(newBlocks, endTextIndex);
    ...
    withParagraphsAsLists(
        ...
        blocksInRange.forEach((block) => {
            if (
                previous === undefined ||
                block.getParent(this) === previous.getParent(this)
            )
    ...
    withListsIndented(
        ...
        for (let i = 0; i < formats.length; i++) {
            ...
            if (first.hasAncestor(newBlocks, format)) inside = true;
            ...
            if (inside) {
                ...
                if (list instanceof ListNode) {
                    const newList = indent
                        ? list.withItemIndented(format)
                        : list.withItemUnindented(format);
            ...
            if (last.hasAncestor(newBlocks, format)) inside = false;
        }
    ...
    withoutAdjacentContent(caret: Caret, next: boolean): Edit | undefined {
        ...
        while (parents && parents.length > 0) {
            ...
            // See if the CodeNode would like to handle it.
                ...
                const newCaret = {
                    node: revisedCode,
                    index: Math.max(0, caret.index + (next ? 0 : -1)),
                };
            ...
            // If it's a format in the list node, merge with the previous list item
                ...
                // Convert the remaining list item into a paragraph.
                    ...
                    parents.length > 0 &&
                    parents[parents.length - 1] instanceof BlocksNode
                ) {
            ...
            // If it's at the beginning of the paragraph, and there's a previous paragraph, merge with the previous paragraph.
                ...
                const firstCaret = next
                    ? parent.getLastCaret()
                    : parent.getFirstCaret();
                if (firstCaret === undefined) return;
                const blocks = parents[parents.length - 1] as BlocksNode;
                const adjacentBlock = next
                    ? blocks.getBlockAfter(parent)
                    : blocks.getBlockBefore(parent);
                // Is this the first/last caret of the paragraph and there's an adjascent block? Try deleting something.
                if (
                    firstCaret.node === caret.node &&
                    firstCaret.index === caret.index &&
                    adjacentBlock !== undefined
                ) {
                    ...
                    if (adjacentBlock instanceof ParagraphNode) {
                        const currentCaret = next
                            ? parent.getLastCaret()
                            : adjacentBlock.getLastCaret();
                        if (currentCaret === undefined) return;
                        const textIndex = next
                        ...
                        const mergedParagraph = next
                            ? adjacentBlock.withParagraphPrepended(parent)
                            : adjacentBlock.withParagraphAppended(parent);
                        ...
                        if (next) {
                            ...
                            if (format !== undefined) {
                                ...
                                const newBlocks = blocks
                                    ...
                                    ?.withNodeReplaced(format, undefined);
                                if (newBlocks === undefined) return;
                                return {
                                    root: newBlocks,
    ...
    withNodeInserted(caret: Caret, node: Node): Edit {
        ...
        while (blocks.length > 0) {
            ...
            if (
                lastInsert === anchor &&
                anchor instanceof ParagraphNode &&
                newBlock instanceof ParagraphNode
            ) {
            ...
            // If we're inserting after the anchor and the anchor is a list and the block to insert is a list, merge them.
                ...
                lastInsert === anchor &&
                anchor instanceof ListNode &&
                newBlock instanceof ListNode
            ) {
                const edit = anchor.withNodeInserted(caret, newBlock);
                if (edit === undefined || !(edit.root instanceof BlockNode))
                    return;
                newBlock = edit.root;
                newBlocks = newBlocks.withChildReplaced(anchor, newBlock);
            }
