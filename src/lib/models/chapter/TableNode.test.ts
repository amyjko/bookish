...
test('Insert', () => {
    expect(table.withNewRow(0)?.toBookdown()).toBe(',|\n,tl|tr\n,bl|br\nhi');
    expect(table.withNewColumn(0)?.toBookdown()).toBe(',|tl|tr\n,|bl|br\nhi');
});

test('Delete', () => {
    expect(table.withoutRow(0)?.toBookdown()).toBe(',bl|br\nhi');
    expect(table.withoutColumn(0)?.toBookdown()).toBe(',tr\n,br\nhi');
});
