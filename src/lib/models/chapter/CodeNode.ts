...
export default class CodeNode extends BlockNode {
    ...
    toHTML(): string {
        return `<pre><code>${this.#code.toHTML()}</code><pre><center>${this.#caption.toHTML()}</center>`;
    }
    ...
    getParentOf(node: Node): Node | undefined {
        return node == this.#code || node === this.#caption
            ? this
            : this.#caption.getParentOf(node);
    }

    withChildReplaced(node: Node, replacement: Node | undefined) {
        const newCode =
            node === this.#code && replacement instanceof TextNode
                ? replacement
                : undefined;
        const newCaption =
            node === this.#caption &&
    ...
    withContentInRange(range: CaretRange): this | undefined {
        if (!this.contains(range.start.node) && !this.contains(range.end.node))
            return this.copy();
        const newCode = this.#caption.contains(range.start.node)
            ? new TextNode()
            : this.#code.withContentInRange(range);
        const newCaption =
            range.end.node === this.#code
                ? new FormatNode('', [new TextNode()])
    ...
    withRangeFormatted(range: CaretRange, format: Format | undefined): Edit {
        ...
        if (captionEdit) editedNode = captionEdit.root as this;
        ...
        if (containsStart) {
            ...
            return {
                root: editedNode,
                range: { start: revisedCaret, end: revisedCaret },
            };
        } else {
            return captionEdit;
        }
