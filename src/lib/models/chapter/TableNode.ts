...
export default class TableNode extends BlockNode {
    ...
    toText() {
        return this.#rows
            .map((row) => row.map((cell) => cell.toText()).join(', '))
            .join(', ');
    }
    ...
    toBookdown(): string {
        return (
            ...
            `\n${this.#position === '|' ? '' : this.#position}${
                this.#caption ? '' + this.#caption.toBookdown() : ''
            }`
        );
    }

    toHTML() {
        return `<table><tbody>${this.#rows
            .map(
                (row) =>
                    `<tr>${row
    ...
    locate(format: FormatNode): Location | undefined {
        for (let r = 0; r < this.#rows.length; r++) {
            ...
            for (let c = 0; c < row.length; c++) {
                if (row[c] === format) return { row: r, column: c };
            }
        }
    }
    ...
    withNewColumn(index: number): TableNode | undefined {
        if (
            this.#rows.length > 0 &&
            (index < 0 || index > this.#rows[0].length)
        )
            return;
    ...
    withContentInRange(range: CaretRange): this | undefined {
        ...
        for (; r < this.#rows.length; r++, c = 0) {
            ...
            if (
                containsStart &&
                !foundStart &&
                row.find((r) => r.contains(range.start.node)) === undefined
            )
                continue;
            // Stop if we found the end row.
            if (containsEnd && foundEnd) break;

            newRows.push([] as FormatNode[]);
            for (; c < row.length; c++) {
                if (!foundStart && row[c].contains(range.start.node))
                    foundStart = true;
    ...
    withPosition(position: Position) {
        return new TableNode(this.#rows, position, this.#caption);
    }
}
