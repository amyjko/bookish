...
export const loadPublishedBooksFromFirestore = async (): Promise<Book[]> => {
    ...
    try {
        ...
        return data.docs
            .map((doc) => new Book(doc.ref, doc.data() as BookSpecification))
...
export const loadUsersBooksFromFirestore = async (
    userID: string
): Promise<Book[] | null> => {
    if (!db)
    ...
    try {
        ...
        return books.docs.map(
            (doc) => new Book(doc.ref, doc.data() as BookSpecification)
        );
    } catch (err) {
        console.error(err);
        return null;
...
export const getBookFromIDOrName = async (
    bookIDOrName: string
): Promise<Book | null> => {
...
export const loadBookFromFirestore = async (
    bookID: string
): Promise<Book | null> => {
    if (!db) return null;
    ...
    return !book.exists()
        ? null
        : new Book(bookRef, book.data() as BookSpecification);
};

export const loadEditionFromFirestore = async (
    book: Book,
    editionID: string
): Promise<Edition> => {
    ...
    if (edition.exists())
        return new Edition(
            book,
            edition.ref,
            edition.data() as EditionSpecification
        );
    else throw Error('' + editionID + " doesn't exist");
};

export const createBookInFirestore = async (
    userID: string
): Promise<string> => {
...
export const publishDraftInFirestore = async (
    book: Book,
    index: number,
    published: boolean
): Promise<void> => {
...
export const updateEditionInFirestore = async (
    edition: Edition
): Promise<void> => {
...
export const deleteEditionInFirestore = async (
    book: Book,
    index: number
): Promise<void> => {
    if (!db) throw Error("Can't update edition, not connected to Firebase.");
...
export const addChapterInFirestore = async (
    edition: DocumentReference,
    chapter: ChapterContent
): Promise<DocumentReference> => {
    if (!db) throw Error("Can't add chapter, not connected to Firebase.");

    return await addDoc(
        collection(db, `editions/${edition.id}/chapters`),
        chapter
    );
};

export const removeChapterFromEditionInFirestore = async (
    chapter: Chapter
): Promise<void> => {
    if (!db) throw Error("Can't add chapter, not connected to Firebase.");
...
export const loadChapterTextFromFirestore = async (
    chapter: DocumentReference
...
export const updateChapterTextInFirestore = async (
    edition: DocumentReference,
    chapter: DocumentReference,
    text: string
): Promise<void> => {
    if (!db) throw Error("Can't update chapter, not connected to Firebase.");
...
export const subdomainIsAvailable = async (
    subdomain: string,
    book: Book
): Promise<boolean> => {
    if (!db) throw Error('Not connected to Firebase.');
...
export const getBookIDFromBookName = async (
    subdomain: string
): Promise<string | null> => {
    if (!db)
