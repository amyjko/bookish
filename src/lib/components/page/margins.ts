...
export function hideOutlineIfObscured() {
    ...
    if (window.getComputedStyle(outline).getPropertyValue('left') !== '0px') {
        ...
        else outline.style.removeProperty('opacity');
    }
...
export function layoutMarginals() {
    ...
    Array.from(marginals).forEach((element: Element) => {
        ...
        if (
            ...
            if (el.parentElement) {
                let containingMarginal =
                    el.parentElement.closest('.bookish-marginal');
                ...
                let parentTop =
                    parentBounds.top +
                    window.scrollY -
                    (book.getBoundingClientRect().top + window.scrollY);
                ...
                if (containingMarginal !== null && currentBottom) {
                    ...
                    el.style.top =
                        '' +
                        (currentBottom -
                            (containingMarginal.getBoundingClientRect().top +
                                window.scrollY)) +
                        'px';
                    ...
                    if (rightInsets.length > 0) {
                        Array.from(rightInsets).forEach((inset) => {
                            ...
                            let elementBottom =
                                elementTop + elementBounds.height;

                            // If they overlap...
                            if (currentBottom !== null) {
                                if (
                                    (elementTop <= insetTop &&
                                        elementBottom >= insetTop) ||
                                    (elementTop <= insetBottom &&
                                        elementBottom >= insetBottom) ||
                                    (elementTop >= insetTop &&
                                        elementBottom <= insetBottom)
                                ) {
                                    ...
                                    let newTop =
                                        ...
                                        (insetBottom - elementTop);
                                    // Reposition the marginal.
                                    el.style.top = newTop + 'px';
                                    // Update the new bottom.
                                    currentBottom =
                                        newTop + elementBounds.height;
                                }
                            }
                        });
                    }
