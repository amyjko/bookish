<script lang="ts">
    ...
    import { caretRangeToIndexRange, indexRangeToCaretRange } from "$lib/models/chapter/Caret";
    ...
    function undo(): Edit {
        ...
        let node = 
            ast instanceof ChapterNode ? Parser.parseChapter($edition, undoState.bookdown) : 
            ast instanceof FormatNode ? Parser.parseFormat($edition, undoState.bookdown) :
            ast instanceof EmbedNode ? Parser.parseEmbed($edition, undoState.bookdown) :
            undefined;
    ...
    function getCaretContext(): CaretState | undefined {
        ...
        const parents = editedNode.getParentsOf(caretRange.start.node)?.reverse() as BookishNode[];
    ...
    function saveEdit(newRoot: RootNode, newRange: CaretRange, command?: Command) {
    ...
    function editNode(previous: BookishNode, edited: BookishNode) {
        if(caretRange) {
            ...
            if(caretRange.start.node === previous && (edited instanceof AtomNode || edited instanceof TextNode))
                caretRange = { start: { node: edited, index: 0 }, end: { node: edited, index: 0 }};
    ...
    $: inAtom = caretRange && caretRange.start.node.isInside(editedNode, AtomNode);
    $: isSelection = caretRange && (caretRange.start.node !== caretRange.end.node || caretRange.start.index !== caretRange.end.index);
    $: isItalic = caretRange && !isSelection && caretRange.start.node instanceof TextNode && caretRange.start.node.isItalic(editedNode);
    $: isBold = caretRange && !isSelection && caretRange.start.node instanceof TextNode && caretRange.start.node.isBold(editedNode);
    $: isLink = caretRange && !isSelection && caretRange.start.node.getClosestParentMatching(editedNode, p => p instanceof LinkNode) !== undefined;
...
>
    ...
    {#if caretCoordinate && caretRange && !isAtom && !isSelection && editorFocused }
        <div 
            class={`bookish-editor-caret ${isLink ? "bookish-editor-caret-linked" : isItalic ? "bookish-editor-caret-italic" :""} ${isBold ? "bookish-editor-caret-bold" : ""} ${editorFocused && keyboardIdle ? "bookish-editor-caret-blink" : ""} ${!editorFocused ? "bookish-editor-caret-disabled" : ""} ${ignoredInput ? "bookish-editor-caret-ignored" : ""}`}
