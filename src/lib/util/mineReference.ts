...
export function mineReference(ids: string[], text: string): ReferenceNode {
	...
	return chunks.length > 0 ? 
		...
		new ReferenceNode(getUniqueReferenceID(ids, authors, year), authors || "", year || "", title || "", source || "", url || "", "", false);

}

export function getUniqueReferenceID(ids: string[], authors?: string, year?: string): string {
